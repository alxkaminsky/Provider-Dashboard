...
class CustomerFilter(Filter):
    ...
    def apply(self, customers: list[Customer],
        ...
        try:
            for c in customers:
                if int(filter_string.strip()) == c.get_id():
                    nums = c.get_phone_numbers()
                    lst = [call for call in data if
                           call.src_number in nums or call.dst_number in nums]
                    return lst
            raise ValueError
        except ValueError:
            return data
...
class DurationFilter(Filter):
    ...
    def apply(self, customers: list[Customer],
        ...
        try:
            ...
            condition = filter_string[0]
            dur = int(filter_string[1:])
...
class LocationFilter(Filter):
    ...
    def apply(self, customers: list[Customer],
        ...
        specified in the handout.
        """
        lat_min, lat_max = 43.576959, 43.799568
        long_min, long_max = -79.697878, -79.196382

        try:
            locs = filter_string.split(',')
            tracker = []
            lst = []
            ...
            for loc in locs:
                lst.append(float(loc.strip()))

            low = (lst[0], lst[1])
            up = (lst[2], lst[3])

            if not (long_min <= low[0] <= long_max
                    and long_min <= up[0] <= long_max
                    and lat_min <= low[1] <= lat_max
                    and lat_min <= up[1] <= lat_max):
                raise ValueError

            for c in data:
                if ((low[0] <= c.dst_loc[0] <= up[0])
                        and (low[1] <= c.dst_loc[1] <= up[1])
                        or (low[0] <= c.src_loc[0] <= up[0])
                        and (low[1] <= c.src_loc[1] <= up[1])):
                    tracker.append(c)
            return tracker
        except ValueError:
            return data
