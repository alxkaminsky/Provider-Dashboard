...
def process_event_history(log: dict[str, list[dict]],
    ...
    billing_date = datetime.datetime.strptime(log['events'][0]['time'],
                                              "%Y-%m-%d %H:%M:%S")
    for d in log['events']:
        nd = datetime.datetime.strptime(d['time'], "%Y-%m-%d %H:%M:%S")
        if nd.month != billing_date.month:
            new_month(customer_list, nd.month, nd.year)
        if d['type'] == 'call':
            c1 = find_customer_by_number(d['src_number'], customer_list)
            c2 = find_customer_by_number(d['dst_number'], customer_list)
            call = Call(d['src_number'], d['dst_number'], nd,
                        d['duration'], d['src_loc'], d['dst_loc'])
            c1.make_call(call)
            c2.receive_call(call)
